...
if(isset($_GET['action']) && trim($_GET['action']) == 'search') {
	...
	$filter[] = array('filter_category' => 'PARTICIPANT');
	...
	if($participantData) {
		for($i = 0; $i < count($participantData['records']); $i++) {
			...
			$participants[$i] = array(
				'<a href="/participant/details.php?code='.trim($item['participant_code']).'" class="'.($item['participant_active'] == 0 ? 'error' : 'success').'">'.trim($item['participant_name']).'</a>',
...
if(count($_POST) > 0 && isset($_POST['import_file'])) {
	...
	if(!isset($_POST['class_list']) || count($_POST['class_list']) == 0) {
		$errorArray['class_list'] = 'Please select at least one class';
		$formValid = false;
	} else {
	...
	if(count($errorArray) == 0 && $formValid == true) {
		...
		$importData = $participantObject->readImport($_POST['class_list']);
