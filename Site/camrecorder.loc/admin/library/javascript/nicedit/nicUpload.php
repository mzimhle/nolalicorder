 ...
 * @version: 0.9.0
 */
if(!is_dir($zfsession->config['path'].'/media/uploaded/')) { 
	mkdir($zfsession->config['path'].'/media/uploaded/', 0777, true);
}
define('NICUPLOAD_PATH', $zfsession->config['path'].'/media/uploaded/'); 
// Set the path (relative or absolute) to
// the directory to save image files
define('NICUPLOAD_URI', 'http://'.$zfsession->config['site'].'/media/uploaded');   
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    */
	...
	if(!file_put_contents($path, file_get_contents($image))) {
		nicupload_error('Server error, failed to move file');
	} else {
		chmod($path, 0777);
	}
