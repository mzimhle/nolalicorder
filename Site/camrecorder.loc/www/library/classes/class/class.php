   ...
   //declare table variables
				...
				if(isset($filter[$i]['filter_search']) && trim($filter[$i]['filter_search']) != '') {
					...
					if(count($array) > 0) {
						...
						for($s = 0; $s < count($array); $s++) {
							$text = $array[$s];
							$this->sanitize($text);
							$where .= "lower(concat_ws(class_name, ' ', department_name)) like lower('%$text%')";
							if(($s+1) != count($array)) {
								$where .= ' or ';
							}
						}
	...
	public function search($term, $limit = 10) {
		...
		$select = $this->_db->select()
			...
			->joinInner(array('account' => 'account'), 'account.account_code = faculty.account_code', array('account_name'))				
			->where('course_deleted = 0 and faculty_deleted = 0 and department_deleted = 0 and qualification_deleted = 0 and class_deleted = 0 and account_deleted = 0 and account.account_code = ?', $this->_account)				
			->where("lower(concat(class_name, course_name, department_name, faculty_name, qualification_name)) like lower(?)", "%$term%")
			->limit($limit)
			->order("LOCATE('$term', concat_ws(class_name, course_name, department_name, faculty_name, qualification_name))");
