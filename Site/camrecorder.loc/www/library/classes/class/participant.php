   ...
   //declare table variables
	...
	public function search($term, $limit = 10, $category = 'PARTICIPANT') {
		$select = $this->_db->select()
			...
			->where("lower(concat(participant_name, participant_email, participant_cellphone)) like lower(?)", "%$term%")
			->limit($limit)
			->order("LOCATE('$term', concat_ws(participant_name, participant_email, participant_cellphone))");
